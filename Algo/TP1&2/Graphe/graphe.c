...
void DFS(grapheNodes_s *nodes)
{
    // EDGE
    char *title = "From Type, From Name, Edge, To Type, To Name";
    FILE *fPtr_edge;
    fPtr_edge = fopen("edges_DFS.csv", "w");
    if (fPtr_edge == NULL)
    {
        printf("Unable to create file.\n");
        exit(EXIT_FAILURE);
    ...
    char *title_node = "Type, Name, Description, Image, Reference";
    FILE *fPtr_node;
    fPtr_node = fopen("nodes_DFS.csv", "w");
    if (fPtr_node == NULL)
    {
        printf("Unable to create file.\n");
        exit(EXIT_FAILURE);
    ...
    while (P != NULL)
    {
        grapheNode *u = searchNode(nodes, ll_lastNode(P)->data);
        int len = u->adj_num;
        grapheNode **voisins = u->grapheNodeAdj;
        for (int i = 0; i < len; i++)
        {
